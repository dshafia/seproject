...
function calculateScore(groupedHeroes, words) {
  const correctOrder = LETTERS.filter(hero => hero.words === words).sort((a, b) =>
    a.name.toLowerCase() < b.name.toLowerCase() ? -1 : 1
  );

  return groupedHeroes.reduce((score, { name }, index) => {
    const maxPoint = LETTERS.length;
...
export function getTotalScore(groups, timeLeft) {
  const gameScore = Object.values(WORDS).reduce(
